...
describe('FrameSelector', () => {
  ...
  it('renders all 6 frame options', () => {
    renderWithProviders(
      <FrameSelector selectedFrame={BadgeFrame.none} onSelectFrame={onSelectFrame} />,
    );

    for (const frame of ALL_FRAMES) {
      expect(screen.getByLabelText(`${FRAME_LABELS[frame]} frame`)).toBeOnTheScreen();
    }
  });
  ...
  it('has radiogroup accessibility on container', () => {
    ...
    expect(screen.getByLabelText('Badge frame').props.accessibilityRole).toBe('radiogroup');
  });
