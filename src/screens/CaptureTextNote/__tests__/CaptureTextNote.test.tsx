...
describe('CaptureTextNote', () => {
  it('renders the screen with header', () => {
    renderWithProviders(<CaptureTextNote route={defaultRoute} navigation={{} as any} />);
  ...
  it('enables Save buttons when content is entered', () => {
    renderWithProviders(<CaptureTextNote route={defaultRoute} navigation={{} as any} />);
    fireEvent.changeText(screen.getByLabelText('Note content'), 'My first note');
  ...
  it('saves evidence with goal attachment when no stepId', () => {
    renderWithProviders(<CaptureTextNote route={defaultRoute} navigation={{} as any} />);
    fireEvent.changeText(screen.getByLabelText('Note content'), 'My learning note');
  ...
  it('saves evidence with step attachment when stepId is provided', () => {
    renderWithProviders(<CaptureTextNote route={routeWithStep} navigation={{} as any} />);
  ...
  it('includes caption as description when provided', () => {
    renderWithProviders(<CaptureTextNote route={defaultRoute} navigation={{} as any} />);
    fireEvent.changeText(screen.getByLabelText('Note content'), 'My note content');
    fireEvent.changeText(screen.getByLabelText('Caption (optional)'), 'My caption');
    fireEvent.press(screen.getByText('Save Note'));
  ...
  it('trims content before saving', () => {
    renderWithProviders(<CaptureTextNote route={defaultRoute} navigation={{} as any} />);
    fireEvent.changeText(screen.getByLabelText('Note content'), '  trimmed note  ');
    fireEvent.press(screen.getByText('Save Note'));
  ...
  it('has accessible character count label', () => {
    renderWithProviders(<CaptureTextNote route={defaultRoute} navigation={{} as any} />);
    fireEvent.changeText(screen.getByLabelText('Note content'), 'Hello');
    expect(screen.getByLabelText('5 of 1000 characters used')).toBeOnTheScreen();
  });
});
