...
export function validateBadge(badge: unknown): ValidationResult {
  ...
  if (!Shared.isJsonLdObject(badge)) {
    result.errors.push(
      "Badge must be a valid JSON-LD object with @context and type properties",
    );
    return result;
  }
  ...
  result.errors.push(
    "Badge is not a valid OB2 Assertion or OB3 VerifiableCredential",
  );
  return result;
}
 ...
 */
function validateOB2Assertion(
  assertion: OB2.Assertion,
  ...
  if (!Shared.isDateTime(assertion.issuedOn)) {
    result.errors.push(
      "Assertion issuedOn must be a valid DateTime in ISO 8601 format",
    );
    result.isValid = false;
  }
  ...
  if (!OB2.isVerificationObject(assertion.verification)) {
    result.errors.push(
      "Assertion verification must be a valid VerificationObject",
    );
    result.isValid = false;
  }
  ...
  if (assertion.expires && !Shared.isDateTime(assertion.expires)) {
    result.warnings.push(
      "Assertion expires should be a valid DateTime in ISO 8601 format",
    );
  }
}
 ...
 * @param result The validation result to update
 */
function validateOB2BadgeClass(
  ...
  if (
    typeof badgeClass.description !== "string" ||
    badgeClass.description.trim() === ""
  ) {
...
function validateOB3VerifiableCredential(
  ...
  if (!OB3.isCredentialSubject(credential.credentialSubject)) {
    result.errors.push(
      "VerifiableCredential credentialSubject must be a valid CredentialSubject",
    );
    result.isValid = false;
  } else {
