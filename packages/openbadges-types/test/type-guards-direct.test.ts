...
describe("Direct Type Guards", () => {
  ...
  describe("OB2 Type Guards", () => {
    ...
    test("isOB2Profile should correctly identify OB2 Profile objects", () => {
      const profile =
        typeof ob2Badge.badge === "object" ? ob2Badge.badge.issuer : null;
  ...
  describe("Shared Type Guards", () => {
    ...
    test("hasJsonLdContext should correctly check for JSON-LD contexts", () => {
      expect(hasJsonLdContext(ob2Badge, "https://w3id.org/openbadges/v2")).toBe(
        true,
      );
      expect(
        hasJsonLdContext(ob3Badge, "https://www.w3.org/2018/credentials/v1"),
      ).toBe(true);
      expect(hasJsonLdContext(ob2Badge, "https://example.org/context")).toBe(
        false,
      );
      expect(hasJsonLdContext(null, "https://w3id.org/openbadges/v2")).toBe(
        false,
      );
    });
