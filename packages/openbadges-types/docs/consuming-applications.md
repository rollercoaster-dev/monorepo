...
export default {
  ...
  setup(props) {
    ...
    const filteredBadges = computed(() => {
      ...
      if (searchTerm.value) {
        result = BadgeNormalizer.filterBadgesBySearchTerm(
          result,
          searchTerm.value,
        );
      }
      ...
      result = BadgeNormalizer.sortBadges(
        result,
        sortField.value,
        sortDirection.value,
      );
...
export class BadgeService {
  ...
  getBadgesByRecipient(recipientId: string) {
    const normalizedBadges = this.getNormalizedBadges();
    return BadgeNormalizer.filterBadgesByRecipient(
      normalizedBadges,
      recipientId,
    );
  }
