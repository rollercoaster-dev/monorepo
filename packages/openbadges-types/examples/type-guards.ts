...
function jsonLdTypeGuardsExample() {
  ...
  console.log(
    "invalidJsonLd is JSON-LD:",
    Shared.isJsonLdObject(invalidJsonLd),
  );
  console.log(
    "arrayTypeJsonLd is JSON-LD:",
    Shared.isJsonLdObject(arrayTypeJsonLd),
  );

  // Check for specific types
  console.log(
    "validJsonLd has type Assertion:",
    Shared.hasJsonLdType(validJsonLd, "Assertion"),
  );
  console.log(
    "arrayTypeJsonLd has type Extension:",
    Shared.hasJsonLdType(arrayTypeJsonLd, "Extension"),
  );
  console.log(
    "validJsonLd has type BadgeClass:",
    Shared.hasJsonLdType(validJsonLd, "BadgeClass"),
  );

  // Check for specific contexts
  console.log(
    "validJsonLd has OB2 context:",
    Shared.hasJsonLdContext(validJsonLd, Shared.OB2Context),
  );
  console.log(
    "validJsonLd has OB3 context:",
    Shared.hasJsonLdContext(validJsonLd, Shared.OB3Context),
  );
}
...
function ob2TypeGuardsExample() {
  ...
  console.log(
    "validAssertion is an OB2 Assertion:",
    OB2.isAssertion(validAssertion),
  );
  console.log(
    "invalidAssertion is an OB2 Assertion:",
    OB2.isAssertion(invalidAssertion),
  );

  // Extract the BadgeClass from the valid assertion
  const badgeClass =
    typeof validAssertion.badge === "string"
...
function ob3TypeGuardsExample() {
  ...
  console.log(
    "validCredential is an OB3 VerifiableCredential:",
    OB3.isVerifiableCredential(validCredential),
  );
  console.log(
    "invalidCredential is an OB3 VerifiableCredential:",
    OB3.isVerifiableCredential(invalidCredential),
  );

  // Extract the Achievement from the valid credential
  const achievement = validCredential.credentialSubject.achievement;
  const singleAchievement = Array.isArray(achievement)
    ? achievement[0]
    : achievement;

  console.log(
    "achievement is an Achievement:",
    OB3.isAchievement(singleAchievement),
  );
...
function sharedTypeGuardsExample() {
  ...
  const invalidIRIs = ["not a url", "", 123, null, undefined, {}];
