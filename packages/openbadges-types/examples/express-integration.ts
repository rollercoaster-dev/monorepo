...
app.post("/badges", (req, res) => {
  ...
  if (!Shared.isJsonLdObject(badgeData)) {
    return res
      .status(400)
      .json({ error: "Invalid badge data: Not a JSON-LD object" });
  }
  ...
  try {
    ...
    if (OB2.isAssertion(badgeData)) {
      ...
      return res
        .status(400)
        .json({ error: "Invalid badge data: Not a recognized badge format" });
    }
  } catch (error) {
    return res
      .status(500)
      .json({ error: "Failed to create badge", details: error.message });
  }
});
...
function createSampleOB2Assertion(): OB2.Assertion {
  ...
  const badgeClass: OB2.BadgeClass = {
    ...
    description:
      "This badge is awarded for creating a RESTful API with Express.",
    image: createIRI("https://example.org/badges/5/image"),
    criteria: {
      narrative:
        "Recipients must create a fully functional RESTful API using Express and Node.js.",
    },
    issuer: {
...
function createSampleOB3Credential(): OB3.VerifiableCredential {
  ...
  const achievement: OB3.Achievement = {
    ...
    criteria: {
      narrative:
        "Recipients must create a fully functional GraphQL API using Apollo Server.",
    },
