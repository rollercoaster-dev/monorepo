...
describe("BadgeList and BadgeDisplay Integration", () => {
  ...
  it("should pass interactive prop from BadgeList to BadgeDisplay", () => {
    ...
    const interactiveBadgeDisplays =
      interactiveWrapper.findAllComponents(BadgeDisplay);
    ...
    const nonInteractiveBadgeDisplays =
      nonInteractiveWrapper.findAllComponents(BadgeDisplay);
  ...
  it("should handle pagination correctly", async () => {
    ...
    expect(badgeDisplays.length).toBe(2);
    expect(badgeDisplays[0].props("badge").id).toBe(
      "http://example.org/badge1",
    );
    expect(badgeDisplays[1].props("badge").id).toBe(
      "http://example.org/badge2",
    );
    ...
    expect(badgeDisplays.length).toBe(2);
    expect(badgeDisplays[0].props("badge").id).toBe(
      "http://example.org/badge3",
    );
    expect(badgeDisplays[1].props("badge").id).toBe(
      "http://example.org/badge4",
    );
  });
