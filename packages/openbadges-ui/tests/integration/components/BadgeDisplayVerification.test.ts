...
describe("BadgeDisplay and BadgeVerification Integration", () => {
  ...
  it("should auto-verify badge when autoVerify is true", async () => {
    ...
    (
      BadgeVerificationService.verifyBadge as unknown as MockedVerifyBadge
    ...
    expect(BadgeVerificationService.verifyBadge).toHaveBeenCalledWith(
      mockBadge,
    );
  ...
  it("should show verification status correctly", async () => {
    ...
    await verificationComponent
      .find("button.ob-badge-verification-button")
      .trigger("click");
    ...
    expect(
      verificationComponent.find(".ob-badge-verification-valid").exists(),
    ).toBe(true);
    expect(
      verificationComponent.find(".ob-badge-verification-valid").text(),
    ).toContain("Verified");
  });
  ...
  it("should show verification details when verification is complete", async () => {
    ...
    expect(
      verificationComponent.find(".ob-badge-verification-method").exists(),
    ).toBe(true);
    expect(
      verificationComponent.find(".ob-badge-verification-method").text(),
    ).toContain("Hosted");

    // Check that warnings are displayed
    expect(
      verificationComponent.find(".ob-badge-verification-warnings").exists(),
    ).toBe(true);
    expect(
      verificationComponent.find(".ob-badge-verification-warnings").text(),
    ).toContain("This is a test warning");
  });
  ...
  it("should handle verification failure correctly", async () => {
    ...
    expect(
      verificationComponent.find(".ob-badge-verification-invalid").exists(),
    ).toBe(true);
    expect(
      verificationComponent.find(".ob-badge-verification-invalid").text(),
    ).toContain("Invalid");

    // Check that errors are displayed
    expect(
      verificationComponent.find(".ob-badge-verification-errors").exists(),
    ).toBe(true);
    expect(
      verificationComponent.find(".ob-badge-verification-errors").text(),
    ).toContain("Invalid badge format");
