 ...
 */
export function createMockOB2Badge(
  overrides: Partial<OB2.Assertion> = {},
): OB2.Assertion {
...
export function createMockOB3Badge(
  ...
  if (overrides.issuer) {
    if (typeof overrides.issuer === "string") {
      ...
      if (
        overrides.issuer.image &&
        typeof overrides.issuer.image === "string"
      ) {
        ...
        // ImageObject
        const imageObjInIssuer =
          issuerObjectInResult.image as Shared.ImageObject;
        if (
          typeof (overrides.issuer.image as Shared.ImageObject).id === "string"
        ) {
          const imageId = (overrides.issuer.image as Shared.ImageObject)
            .id as string;
  ...
  if (overrides.credentialSubject) {
    ...
    if (overrides.credentialSubject.achievement) {
      ...
      const achievementObj = overrides.credentialSubject
        .achievement as OB3.Achievement;
      ...
      if (
        achievementObj &&
        achievementObj.image &&
        typeof achievementObj.image !== "string"
      ) {
  ...
  if (overrides.proof && typeof overrides.proof !== "string") {
    ...
    if (typeof overrides.proof.verificationMethod === "string") {
      proofInResult.verificationMethod = createIRI(
        overrides.proof.verificationMethod,
      );
    }
  }
  ...
  if (
    ...
    typeof overrides.credentialStatus !== "string"
  ) {
