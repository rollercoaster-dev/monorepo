...
export class AccessibilityAudit {
    ...
    textElements.forEach((el) => {
      ...
      if (color === backgroundColor) {
        issues.push({
          severity: "warning",
          message:
            "Text color matches background color, may have insufficient contrast",
  ...
  static checkAriaAttributes(element: HTMLElement): Array<{
    ...
    const elementsWithAriaLabel = element.querySelectorAll(
  ...
  static checkCognitiveAccessibility(element: HTMLElement): Array<{
    ...
    forms.forEach((form) => {
      const instructions = form.querySelectorAll(
        ".ob-form-help, [aria-describedby]",
      );
      if (
  ...
  static checkSensoryAccessibility(element: HTMLElement): Array<{
    ...
    contentAreas.forEach((area) => {
      ...
      const colorElements = area.querySelectorAll(
      ...
      const density =
        (childElements / (areaSize.width * areaSize.height)) * 10000;
