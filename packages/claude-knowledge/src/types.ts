...
export interface TaskSnapshot {
  ...
  taskMetadata?: string;
  /** Optional parent task ID for hierarchical task structures */
  parentTaskId?: string;
...
/**
 * Progress aggregation for a task (including children if any)
 */
export interface TaskProgress {
  /** Total number of tasks (self + children) */
  total: number;
...
/**
 * A task with its children in a hierarchical tree structure
 */
export interface TaskTreeNode {
  ...
  /** Aggregated progress (rolls up from children) */
...
/**
 * Status for a recovered task.
 * Maps to Claude's native task status values.
 */
export type RecoveredTaskStatus = "pending" | "in_progress" | "completed";

/**
 * Type of workflow being recovered.
 */
export type RecoverableWorkflowType =
  | "work-on-issue"
  | "auto-issue"
  | "auto-milestone";

/**
 * A task to be recreated from checkpoint state.
 * Contains all information needed to call TaskCreate/TaskUpdate.
 */
export interface RecoveredTask {
  /** Task subject line (shown in task list) */
  subject: string;
  /** Task description */
  description: string;
  /** Active form text (shown while task is in progress) */
  activeForm: string;
  /** Current task status based on checkpoint phase */
  status: RecoveredTaskStatus;
  /** Indices of tasks in the plan that this task is blocked by */
  blockedByIndices: number[];
  /** Metadata to attach to the task */
  metadata: Record<string, unknown>;
}

/**
 * Plan for recreating tasks from checkpoint state.
 * Generated by task recovery and used to guide TaskCreate/TaskUpdate calls.
 */
export interface TaskRecoveryPlan {
  /** Type of workflow being recovered */
  workflowType: RecoverableWorkflowType;
  /** GitHub issue number (for single-issue workflows) */
  issueNumber?: number;
  /** Milestone name (for milestone workflows) */
  milestoneName?: string;
  /** Workflow ID from checkpoint */
  workflowId: string;
  /** Current phase from checkpoint */
  currentPhase: WorkflowPhase;
  /** Current status from checkpoint */
  currentStatus: WorkflowStatus;
  /** Tasks to recreate, in order (indices used for blockedBy) */
  tasks: RecoveredTask[];
