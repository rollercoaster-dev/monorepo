...
describe("getTranscriptPath", () => {
  it("should return null for empty session ID", async () => {
    const result = await getTranscriptPath("");
  ...
  it("should return null for non-existent transcript", async () => {
    ...
    const result = await getTranscriptPath("test-session-id");
    expect(result).toBeNull();
  });

  it("should return null for invalid session ID format", async () => {
    // Path traversal prevention - rejects session IDs with slashes
    const result = await getTranscriptPath("invalid/session/id");
    expect(result).toBeNull();
  });

  it("should return null for path traversal attempts", async () => {
    // Rejects session IDs that could escape directory structure
    expect(await getTranscriptPath("../../etc/passwd")).toBeNull();
    expect(await getTranscriptPath("..")).toBeNull();
    expect(await getTranscriptPath("foo/../bar")).toBeNull();
  });
});
...
describe("findTranscriptByTimeRange", () => {
  it("should return empty array when no transcripts in time range", async () => {
    ...
    const result = await findTranscriptByTimeRange(startTime, endTime);
    expect(result).toEqual([]);
  });

  it("should return empty array when .claude/projects does not exist", async () => {
  ...
  describe("with transcript files", () => {
    ...
    it("should find transcripts modified within time range", async () => {
      const now = Date.now();
      const oneHourAgo = now - 60 * 60 * 1000;
    ...
    it("should exclude transcripts outside time range", async () => {
      const now = Date.now();
    ...
    it("should return transcripts sorted by modification time (oldest first)", async () => {
      const now = Date.now();
    ...
    it("should handle empty time range", async () => {
      ...
      const result = await findTranscriptByTimeRange(
        specificTime,
        specificTime,
      );
