  ...
  try {
    ...
    try {
      ...
      if (syncResult.success) {
        logger.info('User synced with badge server', { created: syncResult.created })
      } else {
        logger.warn('Failed to sync user with badge server', { error: syncResult.error })
      }
    } catch (syncError) {
...
oauthRoutes.get('/user/:userId/providers', requireAuth, async c => {
  try {
    ...
    logger.error('Failed to get user OAuth providers', { error })
    return c.json(
      {
        success: false,
        error: 'Failed to get OAuth providers',
