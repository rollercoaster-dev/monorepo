...
export class UserSyncService {
  ...
  async getBadgeServerUser(identifier: string): Promise<BadgeServerUser | null> {
    try {
      ...
      logger.error('Error getting badge server user', { error, identifier })
      return null
    }
  }
  ...
  async createBadgeServerUser(user: User): Promise<BadgeServerUser | null> {
    try {
      ...
      logger.error('Error creating badge server user', { error, username: user.username })
      return null
    }
  }
  ...
  async updateBadgeServerUser(
    ...
    try {
      ...
      logger.error('Error updating badge server user', {
        error,
        badgeServerUserId,
        username: user.username,
      })
  ...
  async syncUser(user: User): Promise<UserSyncResult> {
    try {
      ...
      logger.error('Error syncing user', { error, userId: user.id, username: user.username })
      return {
        success: false,
  ...
  async syncUserPermissions(user: User): Promise<boolean> {
    try {
      ...
      logger.error('Error syncing user permissions', { error, userId: user.id })
      return false
    }
  }
  ...
  async getBadgeServerUserProfile(userId: string): Promise<BadgeServerUser | null> {
    try {
      ...
      logger.error('Error getting badge server user profile', { error, userId })
      return null
    }
  }
}
