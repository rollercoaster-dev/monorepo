...
app.use('*', async (c, next) => {
  if (
    ...
    !c.req.header('x-forwarded-proto')?.includes('https')
  ) {
...
if (basicAuthUser === 'admin' && basicAuthPass === 'admin-user') {
  console.warn(
    'WARNING: Using default Basic Auth credentials. Set OPENBADGES_BASIC_AUTH_USER and OPENBADGES_BASIC_AUTH_PASS environment variables.'
  )
}
...
app.use(
  ...
  cors({
    origin: origin => {
      const allowedOrigins = ['http://localhost:7777', 'https://yourdomain.com']
