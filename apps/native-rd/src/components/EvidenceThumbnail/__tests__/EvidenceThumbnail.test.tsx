...
describe('EvidenceThumbnail', () => {
  ...
  it('renders photo thumbnail with image preview', () => {
    const evidence: Evidence = { id: '1', title: 'My photo', type: 'photo', uri: '/photo.jpg' };
  ...
  it('renders text thumbnail with text snippet', () => {
    const evidence: Evidence = { id: '1', title: 'My progress notes', type: 'text' };
    render(<EvidenceThumbnail evidence={evidence} />);
  ...
  it('renders voice memo with icon', () => {
    const evidence: Evidence = { id: '1', title: 'Voice memo', type: 'voice_memo' };
    render(<EvidenceThumbnail evidence={evidence} />);
    expect(screen.getByLabelText('voice_memo evidence: Voice memo')).toBeTruthy();
  });

  it('renders link with URL display', () => {
    const evidence: Evidence = { id: '1', title: 'Example', type: 'link', uri: 'https://example.com' };
    render(<EvidenceThumbnail evidence={evidence} />);
  ...
  it('falls back to icon when photo image fails to load', () => {
    const evidence: Evidence = { id: '1', title: 'Broken photo', type: 'photo', uri: '/invalid.jpg' };
    const { UNSAFE_getAllByType } = render(<EvidenceThumbnail evidence={evidence} />);
  ...
  it('falls back to icon when video thumbnail fails to load', () => {
    const evidence: Evidence = { id: '1', title: 'Broken video', type: 'video', uri: '/invalid.mp4' };
    const { UNSAFE_getAllByType } = render(<EvidenceThumbnail evidence={evidence} />);
  ...
  it('handles invalid JSON metadata gracefully', () => {
    ...
    expect(() => render(<EvidenceThumbnail evidence={evidence} />)).not.toThrow();
  ...
  it('calls onPress when provided', () => {
    ...
    const evidence: Evidence = { id: '1', title: 'Photo', type: 'photo', uri: '/photo.jpg' };
  ...
  it('shows alert when link cannot be opened', async () => {
    ...
    const evidence: Evidence = { id: '1', title: 'Bad link', type: 'link', uri: 'invalid://url' };
    render(<EvidenceThumbnail evidence={evidence} />);
    fireEvent.press(screen.getByLabelText('link evidence: Bad link'));
    await waitFor(() => {
      expect(alertSpy).toHaveBeenCalledWith('Cannot open link', 'Unable to open: invalid://url');
    });
  });
  ...
  it('shows alert when link opening throws error', async () => {
    ...
    const evidence: Evidence = { id: '1', title: 'Link', type: 'link', uri: 'https://example.com' };
    render(<EvidenceThumbnail evidence={evidence} />);
    fireEvent.press(screen.getByLabelText('link evidence: Link'));
    await waitFor(() => {
      expect(alertSpy).toHaveBeenCalledWith('Cannot open link', 'Failed to open: https://example.com');
    });
  });
