...
describe('Checkbox', () => {
  ...
  it('renders label text', () => {
    renderWithProviders(<Checkbox checked={false} onToggle={jest.fn()} label="Accept terms" />);
    expect(screen.getByText('Accept terms')).toBeOnTheScreen();
  });

  it('shows checkmark when checked=true', () => {
    renderWithProviders(<Checkbox checked={true} onToggle={jest.fn()} label="Done" />);
    expect(screen.getByText('✓')).toBeOnTheScreen();
  });

  it('does not show checkmark when checked=false', () => {
    renderWithProviders(<Checkbox checked={false} onToggle={jest.fn()} label="Done" />);
    expect(screen.queryByText('✓')).toBeNull();
  });

  it('calls onToggle when the checkbox box is pressed', () => {
    const onToggle = jest.fn();
    renderWithProviders(<Checkbox checked={false} onToggle={onToggle} label="Accept terms" />);
  ...
  it('calls onLabelPress (not onToggle) when label is pressed and onLabelPress is provided', () => {
    ...
    renderWithProviders(
      <Checkbox checked={false} onToggle={onToggle} label="Step title" onLabelPress={onLabelPress} />,
    );
  ...
  it('has accessibilityRole="checkbox" on the box', () => {
    renderWithProviders(<Checkbox checked={false} onToggle={jest.fn()} label="Check me" />);
  ...
  it('sets label accessibilityLabel to "Edit {label}" when onLabelPress is provided', () => {
    renderWithProviders(
      <Checkbox checked={false} onToggle={jest.fn()} label="My step" onLabelPress={jest.fn()} />,
    );
