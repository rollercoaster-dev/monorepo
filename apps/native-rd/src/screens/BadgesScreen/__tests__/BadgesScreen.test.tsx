...
describe('BadgesScreen', () => {
  describe('empty state', () => {
    it('renders empty state when no badges exist', () => {
      ...
      expect(
        screen.getByText('Complete goals to earn badges. Your collection will grow here.'),
      ).toBeOnTheScreen();
    });
    ...
    it('navigates to Goals tab when Go to Goals is pressed', () => {
      ...
      expect(mockNavigate).toHaveBeenCalledWith('GoalsTab', { screen: 'Goals' });
    });
  });
  ...
  describe('badge list', () => {
    ...
    it('navigates to BadgeDetail when a badge card is pressed', () => {
      const badges = [makeBadgeRow({ id: 'badge-1', goalTitle: 'Learn TypeScript' })];
      mockUseQuery.mockReturnValue(badges);

      renderWithProviders(<BadgesScreen />);
      fireEvent.press(screen.getByText('Learn TypeScript'));
      expect(mockNavigate).toHaveBeenCalledWith('BadgeDetail', { badgeId: 'badge-1' });
    });
