...
jest.mock('../../../utils/fileStorage', () => ({
  saveFileToAppStorage: (...args: unknown[]) => mockSaveFileToAppStorage(...args),
...
function renderScreen(params?: { goalId: string; stepId?: string }) {
  ...
  return renderWithProviders(<CaptureFile route={route} navigation={{} as never} />);
}
...
describe('CaptureFile', () => {
  ...
  it('picks file and creates evidence with goalId', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockSaveFileToAppStorage).toHaveBeenCalledWith('file:///tmp/doc.pdf', 'doc.pdf');
  ...
  it('shows alert when file validation fails', async () => {
    ...
    mockValidateFile.mockReturnValue('File is too large. Maximum size is 50 MB.');
    mockGetDocumentAsync.mockResolvedValue({
      canceled: false,
      assets: [{
