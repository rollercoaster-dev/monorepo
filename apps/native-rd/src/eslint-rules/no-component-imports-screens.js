...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        "Disallow components and utils from importing screens — enforces dependency direction",
    },
    messages: {
      noScreenImport: [
  ...
  create(context) {
    ...
    function check(node, importPath) {
      ...
      const isAliasScreenImport =
        importPath.startsWith("@/screens") ||
        importPath.startsWith("src/screens");
      if (isRelativeScreenImport || isAliasScreenImport) {
        context.report({
          node,
          messageId: "noScreenImport",
