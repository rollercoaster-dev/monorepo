...
describe('ShapeSelector', () => {
  ...
  it('renders all 6 shape options', () => {
    renderWithProviders(
      <ShapeSelector selectedShape={BadgeShape.circle} onSelectShape={onSelectShape} />,
    );

    for (const shape of ALL_SHAPES) {
      expect(screen.getByLabelText(`${SHAPE_LABELS[shape]} shape`)).toBeOnTheScreen();
    }
  });
  ...
  it('has radiogroup accessibility on container', () => {
    ...
    expect(screen.getByLabelText('Badge shape').props.accessibilityRole).toBe('radiogroup');
  });
