...
describe('Banner', () => {
  ...
  it('returns null when banner text is empty', () => {
    expect(Banner(makeProps({ banner: { text: '', position: 'center' } }))).toBeNull();
  });
  ...
  it('shadow rect is offset by BANNER_SHADOW_OFFSET', () => {
    ...
    const mainRect = rects[1];
    expect(shadowRect.props.x).toBe(Number(mainRect.props.x) + BANNER_SHADOW_OFFSET);
    expect(shadowRect.props.y).toBe(Number(mainRect.props.y) + BANNER_SHADOW_OFFSET);
  });
  ...
  it('positions text at vertical center of banner', () => {
    ...
    const rects = findByType(el, 'Rect');
    const mainRect = rects[1];
    expect(texts[0].props.y).toBe(Number(mainRect.props.y) + Number(mainRect.props.height) / 2);
  });
  ...
  it('trims whitespace from banner text', () => {
    const el = Banner(makeProps({ banner: { text: '  HELLO  ', position: 'center' } }))!;
    const texts = findByType(el, 'Text');
