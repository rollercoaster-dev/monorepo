...
describe('Naming conventions', () => {
  ...
  describe('Test files', () => {
    ...
    test.each(components)('%s test file follows <Name>.test.tsx pattern (if tests exist)', (name) => {
  ...
  describe('Hook files', () => {
    test('all hooks in src/hooks/ start with "use"', () => {
      ...
      const hookFiles = fs.readdirSync(HOOKS_DIR).filter((f) => {
        return (f.endsWith('.ts') || f.endsWith('.tsx')) && !f.endsWith('.test.ts') && !f.endsWith('.test.tsx');
      });
