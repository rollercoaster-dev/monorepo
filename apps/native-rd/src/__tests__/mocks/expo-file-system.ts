...
export const writeAsStringAsync = jest.fn(
  (_fileUri: string, _contents: string, _options?: { encoding?: string }): Promise<void> =>
    Promise.resolve(),
);

export const deleteAsync = jest.fn(
  (_fileUri: string, _options?: { idempotent?: boolean }): Promise<void> => Promise.resolve(),
);
...
export const makeDirectoryAsync = jest.fn(
  (_fileUri: string, _options?: { intermediates?: boolean }): Promise<void> => Promise.resolve(),
);
