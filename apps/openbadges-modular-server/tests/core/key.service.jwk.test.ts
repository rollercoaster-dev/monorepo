...
describe("KeyService JWK Functionality", () => {
  ...
  describe("JWK Conversion", () => {
    ...
    it("should throw error for unsupported key type", async () => {
      ...
      expect(() => {
        ...
        new RegExp(
          `Unsupported key type for JWK conversion: ${unsupportedKeyType}`,
        ),
      );
    });
  });
  ...
  describe("Key Rotation", () => {
    it("should rotate a key successfully", async () => {
      // Generate a test key
      const originalKey = await KeyService.generateKeyPair(
        "rotate-test",
        KeyType.RSA,
      );
      ...
      const newKeyExists = Array.from(statusInfo.keys()).some(
        (id) =>
          id.startsWith("rotate-test-") &&
          statusInfo.get(id)?.status === KeyStatus.ACTIVE,
      );
  ...
  describe("Key Status Management", () => {
    ...
    it("should throw error when setting status for non-existent key", async () => {
      await expect(
        KeyService.setKeyStatus("non-existent", KeyStatus.INACTIVE),
