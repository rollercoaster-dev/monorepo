...
export class TestDataHelper {
  ...
  static async createIssuer(
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          `Failed to create issuer: ${res.status} ${res.statusText} - ${responseText}`,
        );
      }
  ...
  static async createBadgeClass(
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Failed to create badge class: ${res.status} ${res.statusText} - ${errorText}`,
      );
    }
  ...
  static async createAssertion(
    ...
    if (!res.ok) {
      const errorText = await res.text();
      throw new Error(
        `Failed to create assertion: ${res.status} ${res.statusText} - ${errorText}`,
      );
    }
  ...
  private static async cleanupEntities(entityType: string): Promise<void> {
    ...
    for (const id of ids) {
      try {
        ...
        const endpoint =
          entityType === "badgeClasses" ? "badge-classes" : entityType;
