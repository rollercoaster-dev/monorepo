...
(runTests ? describe : describe.skip)("PostgresBadgeClassRepository", () => {
  ...
  it("should create a badge class", async () => {
    ...
    expect(createdBadgeClass.image.toString()).toBe(
  ...
  it("should update a badge class", async () => {
    ...
    expect(updatedBadgeClass?.criteria).toEqual({
      narrative: "Updated criteria",
    });
  ...
  it("should find badge classes by issuer", async () => {
    ...
    const anotherIssuer = await issuerRepository.create(
      Issuer.create({
        name: "Another Issuer",
