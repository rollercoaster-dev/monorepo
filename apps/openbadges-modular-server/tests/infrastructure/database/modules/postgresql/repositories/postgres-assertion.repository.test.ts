...
(runTests ? describe : describe.skip)("PostgresAssertionRepository", () => {
  ...
  beforeEach(async () => {
    ...
    testIssuer = await issuerRepository.create(
      Issuer.create({
        name: "Test Issuer",
    ...
    testBadgeClass = await badgeClassRepository.create(
      BadgeClass.create({
  ...
  it("should update an assertion", async () => {
    ...
    expect(updatedAssertion?.evidence).toEqual([
      { narrative: "Updated evidence" },
    ]);
  ...
  it("should find all assertions", async () => {
    ...
    expect(
      assertions.some((a) => a.recipient.identity === "recipient1@example.com"),
    ).toBe(true);
    expect(
      assertions.some((a) => a.recipient.identity === "recipient2@example.com"),
    ).toBe(true);
  });

  it("should find assertions by badge class", async () => {
    // Create another badge class
    const anotherBadgeClass = await badgeClassRepository.create(
      BadgeClass.create({
        issuer: testIssuer.id,
    ...
    const testBadgeAssertions = await repository.findByBadgeClass(
      testBadgeClass.id,
    );
    const anotherBadgeAssertions = await repository.findByBadgeClass(
      anotherBadgeClass.id,
    );

    // Verify assertions were found correctly
    expect(testBadgeAssertions).toBeDefined();
    expect(testBadgeAssertions.length).toBe(1);
    expect(testBadgeAssertions[0].recipient.identity).toBe(
      "test-badge@example.com",
    );

    expect(anotherBadgeAssertions).toBeDefined();
    expect(anotherBadgeAssertions.length).toBe(1);
    expect(anotherBadgeAssertions[0].recipient.identity).toBe(
      "another-badge@example.com",
    );
  });
  ...
  it("should find assertions by recipient", async () => {
    ...
    const recipientAssertions = await repository.findByRecipient(
      "recipient-search@example.com" as Shared.IRI,
    );

    // Verify assertions were found correctly
    expect(recipientAssertions).toBeDefined();
    expect(recipientAssertions.length).toBe(1);
    expect(recipientAssertions[0].recipient.identity).toBe(
  ...
  it("should revoke an assertion", async () => {
    ...
    const revokedAssertion = await repository.revoke(
      createdAssertion.id,
