 ...
 */
export function validateCreateIssuerDto(
  data: unknown,
): asserts data is CreateIssuerDto {
  ...
  if (missingFields.length > 0) {
    logger.error("Missing required fields in CreateIssuerDto", {
      missingFields,
    });
 ...
 * @throws MissingRequiredFieldsError if required fields are missing
 */
export function validateCreateBadgeClassDto(
  data: unknown,
): asserts data is CreateBadgeClassDto {
  ...
  if (missingFields.length > 0) {
    logger.error("Missing required fields in CreateBadgeClassDto", {
      missingFields,
    });
    throw new MissingRequiredFieldsError(missingFields);
  }
 ...
 * @throws MissingRequiredFieldsError if required fields are missing
 */
export function validateCreateAssertionDto(
  data: unknown,
): asserts data is CreateAssertionDto {
  ...
  if (dto.recipient) {
    ...
    if (dto.recipient.hashed === undefined)
      missingFields.push("recipient.hashed");
  }

  if (missingFields.length > 0) {
    logger.error("Missing required fields in CreateAssertionDto", {
      missingFields,
    });
    throw new MissingRequiredFieldsError(missingFields);
  }
 ...
 */
export function isUpdateBadgeClassDto(
  data: unknown,
): data is UpdateBadgeClassDto {
  return typeof data === "object" && data !== null;
}
 ...
 */
export function isUpdateAssertionDto(
  data: unknown,
): data is UpdateAssertionDto {
  return typeof data === "object" && data !== null;
}
 ...
 * @throws MissingRequiredFieldsError if required fields are missing
 */
export function validateBatchCreateCredentialsDto(
  data: unknown,
): asserts data is BatchCreateCredentialsDto {
  ...
  if (missingFields.length > 0) {
    logger.error("Missing required fields in BatchCreateCredentialsDto", {
      missingFields,
    });
    throw new MissingRequiredFieldsError(missingFields);
  }
 ...
 * @throws MissingRequiredFieldsError if required fields are missing
 */
export function validateBatchRetrieveCredentialsDto(
  data: unknown,
): asserts data is BatchRetrieveCredentialsDto {
  ...
  if (missingFields.length > 0) {
    logger.error("Missing required fields in BatchRetrieveCredentialsDto", {
      missingFields,
    });
    throw new MissingRequiredFieldsError(missingFields);
  }
 ...
 * @throws MissingRequiredFieldsError if required fields are missing
 */
export function validateBatchUpdateCredentialStatusDto(
  data: unknown,
): asserts data is BatchUpdateCredentialStatusDto {
  ...
  if (!dto.updates) {
    ...
    dto.updates.forEach((update, index) => {
      ...
      if (
        !update.status ||
        !["revoked", "suspended", "active"].includes(update.status)
      ) {
        missingFields.push(
          `updates[${index}].status (must be 'revoked', 'suspended', or 'active')`,
        );
      }
    });
  }
  ...
  if (missingFields.length > 0) {
    logger.error("Missing required fields in BatchUpdateCredentialStatusDto", {
      missingFields,
    });
    throw new MissingRequiredFieldsError(missingFields);
  }
