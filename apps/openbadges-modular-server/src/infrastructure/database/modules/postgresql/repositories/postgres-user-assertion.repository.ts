  ...
  implements UserAssertionRepository
{
  ...
  private async createUserAssertion(
    ...
    if (obj.metadata)
      insertValues.metadata = obj.metadata as Record<string, unknown> | null;
    ...
    if (obj.metadata)
      updateValues.metadata = obj.metadata as Record<string, unknown> | null;
  ...
  async removeAssertion(
    ...
    try {
      ...
      logger.error(
        "Error removing assertion from user in PostgreSQL repository",
        {
          error: error instanceof Error ? error.message : String(error),
          userId,
          assertionId,
  ...
  async hasAssertion(
    ...
    try {
      ...
      logger.error(
        "Error checking if user has assertion in PostgreSQL repository",
        {
          error: error instanceof Error ? error.message : String(error),
          userId,
          assertionId,
  ...
  async findByUserAndAssertion(
    ...
    try {
      ...
      logger.error(
        "Error finding user assertion by user and assertion ID in PostgreSQL repository",
        {
          error: error instanceof Error ? error.message : String(error),
          userId,
          assertionId,
  ...
  private rowToDomain(row: unknown): UserAssertion {
    ...
    return UserAssertion.create({
      ...
      addedAt:
        typedRow.addedAt instanceof Date
          ? typedRow.addedAt
          : new Date(String(typedRow.addedAt)),
