...
export function requireAuth(): MiddlewareHandler<{
  ...
  return createMiddleware<{
    ...
    if (
      ...
      config.auth.disableRbac
    ) {
      logger.debug(
        "RBAC is disabled for testing, skipping authentication check",
        logContext,
      );
      await next();
      return;
...
export function requireRoles(roles: UserRole[]): MiddlewareHandler<{
  ...
  return createMiddleware<{
    ...
    if (
      ...
      logger.debug(
        "RBAC is disabled for testing, skipping roles check",
        logContext,
      );
      await next();
      return;
...
export function requirePermissions(
  ...
  return createMiddleware<{
    ...
    if (
      ...
      logger.debug(
        "RBAC is disabled for testing, skipping permissions check",
        logContext,
      );
      await next();
      return;
    ...
    if (requireAll) {
      ...
      logger.debug(
        `Checking if user has ALL required permissions: ${hasRequiredPermissions}`,
        authLogContext,
      );
    } else {
      ...
      logger.debug(
        `Checking if user has ANY required permission: ${hasRequiredPermissions}`,
        authLogContext,
      );
    }
...
export function requireSelfOrAdmin(): MiddlewareHandler<{
  ...
  return createMiddleware<{
    ...
    if (
      ...
      logger.debug(
        "RBAC is disabled for testing, skipping self/admin check",
        logContext,
      );
      await next();
      return;
    ...
    if (!isAdmin && !isSelf) {
      logger.warn(
        "Access denied: User attempted to access resource belonging to another user",
        checkLogContext,
      );
    ...
    logger.info(
      `Self/Admin check passed: Access granted due to ${accessReason}`,
      checkLogContext,
    );
