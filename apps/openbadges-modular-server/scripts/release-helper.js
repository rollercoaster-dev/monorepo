...
function getCommitMessagesSinceTag(tag) {
  ...
  return exec(
    `git log ${range} --oneline --grep="chore(release):" --invert-grep`,
  );
}
...
function checkReleaseNeeded() {
  ...
  if (!isOnMainBranch()) {
    log(
      "❌ Not on main branch. Releases should only be created from main.",
      colors.red,
    );
    return false;
  }
  ...
  if (commitsSinceTag === "0") {
    log(
      "✅ No new commits since last release. No release needed.",
      colors.green,
    );
    return false;
  }
...
function showReleaseOptions() {
  ...
  log(
    "1. Automatic release (recommended): bun run release:manual",
    colors.blue,
  );
...
function validateEnvironment() {
  ...
  try {
    ...
    if (process.env.CI || process.env.GITHUB_ACTIONS) {
      log(
        "ℹ️  Running in CI environment - git config may not be required",
        colors.blue,
      );
...
function main() {
  ...
  try {
    ...
    log(
      "❌ Must be run from project root (package.json not found)",
      colors.red,
    );
    process.exit(1);
  }
