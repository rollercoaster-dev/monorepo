...
class WorkflowValidator {
  ...
  validateSemanticReleaseConfig() {
    ...
    try {
      ...
      if (config.branches && Array.isArray(config.branches)) {
        ...
        const hasMain = config.branches.some(
          (branch) =>
            branch === "main" ||
            (typeof branch === "object" && branch.name === "main"),
        );
    ...
    tests.forEach((test) => {
      try {
        ...
        if (test.required) {
          ...
          this.addWarning(
            `${test.name} failed (non-critical): ${error.message}`,
          );
        }
      }
    });
  ...
  generateReport() {
    ...
    if (isValid) {
      ...
      log(
        "Please fix the errors before deploying the new workflow.",
        colors.red,
      );
    }
